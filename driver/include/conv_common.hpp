...
template <class InDesc,
          ...
          class LowerPads,
          class UpperPads>
constexpr auto get_convolution_output_default_5d_tensor_descriptor(
    InDesc, WeiDesc, ConvStrides, ConvDilations, LowerPads, UpperPads)
{
    ...
    static_assert(in_desc.GetNumOfDimension() == 5, "input nDim is not 5");
    static_assert(wei_desc.GetNumOfDimension() == 5, "weight nDim is not 5");
    ...
    constexpr index_t Di = in_desc.GetLength(I2);
    constexpr index_t Hi = in_desc.GetLength(I3);
    constexpr index_t Wi = in_desc.GetLength(I4);

    constexpr index_t K = wei_desc.GetLength(I0);
    constexpr index_t Z = wei_desc.GetLength(I2);
    constexpr index_t Y = wei_desc.GetLength(I3);
    constexpr index_t X = wei_desc.GetLength(I4);

    constexpr index_t DPadLow = LowerPads{}.Get(I0);
    constexpr index_t HPadLow = LowerPads{}.Get(I1);
    constexpr index_t WPadLow = LowerPads{}.Get(I2);

    constexpr index_t DPadUp = UpperPads{}.Get(I0);
    constexpr index_t HPadUp = UpperPads{}.Get(I1);
    constexpr index_t WPadUp = UpperPads{}.Get(I2);

    constexpr index_t ZEff = (Z - 1) * ConvDilations{}[0] + 1;
    constexpr index_t YEff = (Y - 1) * ConvDilations{}[1] + 1;
    constexpr index_t XEff = (X - 1) * ConvDilations{}[2] + 1;

    constexpr index_t Do = (Di + DPadLow + DPadUp - ZEff) / ConvStrides{}[0] + 1;
    constexpr index_t Ho = (Hi + HPadLow + HPadUp - YEff) / ConvStrides{}[1] + 1;
    constexpr index_t Wo = (Wi + WPadLow + WPadUp - XEff) / ConvStrides{}[2] + 1;

    return make_ConstantTensorDescriptor_packed(Sequence<N, K, Do, Ho, Wo>{});
